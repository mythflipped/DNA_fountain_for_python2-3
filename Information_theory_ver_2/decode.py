    ...
    def add_dna(self, dna_string):
        ...
        if self.rs > 0:
            ...
            if self.correct: #we want to evaluate the error correcting code
                try:
                    #print(data)
                    aa=self.RSCodec.decode(data)
                    #data_corrected = list(aa[0])
                    data_corrected=list(aa)
...
if __name__=='__main__':
    f = '50-SF.txt'
    o = 'surfing.jpg'
    Decode(header_size = 4, rs = 5, delta = 0.05, c_dist = 0.1, chunk_num = 1494, max_homopolymer = 3, gc = 0.05,
           max_hamming = 100, file_in = f, out = o,size=100).main()
